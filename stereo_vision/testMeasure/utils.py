...
def get_img_boxes(flag):
    ...
    tfmodel = os.path.dirname(os.path.realpath(__file__)).replace("testMeasure","tf-faster-rcnn/output/res101/voc_2007_trainval/"
                                                                                "default/res101_faster_rcnn_iter_15000.ckpt")
...
def boxes_saveInfo(flag,boxes_list):
    ...
    for i in range(4):
        ...
        command_path = os.path.dirname(os.path.realpath(__file__)).replace("testMeasure",
                                                                        "static/res_pictures/result/") + flag + "/" + im_names[i] + "/boxes_img/"
        command = "rm -f "+command_path+"*.jpg"
        os.system(command)
