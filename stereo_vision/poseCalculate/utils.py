...
from stereo_vision.prioriImport.utils import fitCircle3D
...
def tubePoseCalculate(filePath,dataPath):
    ...
    if numsOfAquadrant!=0:
        ...
        if numsOfAmouting+numsOfAtapped+numsOfAquadrant!=numsOfAhole+1:
            yon = False
            xx = np.zeros(numsOfAhole)
            yy = np.zeros(numsOfAhole)
            zz = np.zeros(numsOfAhole)
            for i in range(numsOfAhole):
                xx[i] = hole_list[i][0]
                yy[i] = hole_list[i][1]
                zz[i] = hole_list[i][2]
            xc_2A,yc_2A,zc_2A,R_2A,residu_2A = fitCircle3D(xx,yy,zz)
    ...
    if numsOfBquadrant != 0:
        ...
        if numsOfBmouting + numsOfBtapped + numsOfBquadrant != numsOfBhole + 1:
            yon = False
            xx = np.zeros(numsOfBhole)
            yy = np.zeros(numsOfBhole)
            zz = np.zeros(numsOfBhole)
            for i in range(numsOfBhole):
                xx[i] = hole_list[i][0]
                yy[i] = hole_list[i][1]
                zz[i] = hole_list[i][2]
            xc_2B, yc_2B, zc_2B, R_2B, residu_2B = fitCircle3D(xx, yy, zz)
